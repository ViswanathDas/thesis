...
% %         if S1.apf_obst1(i,j)>2000
% %             S1.apf_obst1(i,j)= 2000;
% %         end
% %     end
% % end
...
% s.FaceColor = 'interp';
% % s.FaceAlpha = 0.8;
% % c= contour3(X_var,Y_var,S1.apf_obst1, 100, 'k');
% % hold all;
% % % for i= 1:length(Y_var)
% % %     for j=1:length(X_var)
% % %         if S2.apf_obst1(i,j)>300
% % %             S2.apf_obst1(i,j)= 300;
% % %         end
% % %     end
% % % end
% % s=surf(X_var,Y_var,S2.apf_obst1);
% % % hold on;
% % s.EdgeColor = 'none';
% % s.FaceColor = 'interp';
...
% % Y= linespace(-1,0.01,10000);
% for i= 1:1:width(X)
%     yuk(i)= (A*exp(-b*X(i)))/X(i);
%     modyuk(i)= -(A*log(b*X(i)))/(b*X(i));
% end
% 
% figure(50)
% hold on;
% plot(X, yuk, 'r' ,'DisplayName', '2D representation of Yukawa Potential');
% plot(X, modyuk, 'b', 'DisplayName', '2D representation of Modified Potential')
% legend
% xlim([-1 11])
% ylim([-1 1000])
...
function [apf_obst_fin]= U_obst(const_o, x_o_0, x_h_0, select, distance, size_veh, check)
