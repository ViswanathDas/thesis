...
distance= [1; 1]; % This is the distance that the third vertex of the triangle
...
for j= 1:length(Y_var)
    for k=1:length(X_var)
        ...
        for i= 1:1:width(x_o_0)
            if x_o_0(6,i)>0
                flag1(1,i)= region1(si_form(:,i), x_h);
...
%                     count2= count2+1;
            ...
            elseif x_o_0(6,i)==0
                flag1(1,i)= region2(si_form(:,i), x_h, v_alp1{:,i});
                if flag1(1,i)==flag(1,i)
%                     count1= count1+1;
                    count1(1,i)= count1(1,i)+1;
                    X_vartempin(i,count1(1,i))= x_h(2,1);
...
%                     count2= count2+1;
            ...
            elseif x_o_0(6,i)<0
                flag1(1,i)= region3(si_form(:,i), x_h);
                if flag1(1,i)==flag(1,i)
%                     count1= count1+1;
                    count1(1,i)= count1(1,i)+1;
                    X_vartempin(i,count1(1,i))= x_h(2,1);
...
hold on;
for i= 1:1:width(x_o_0)
plot(X_vartempout(i,:), Y_vartempout(i,:), 'b*')
hold on;
plot(X_vartempin(i,:), Y_vartempin(i,:), 'g*')
end
for i= 1:1:width(x_o_0)
plot(X_vartempin(i,:), Y_vartempin(i,:), 'g*')
hold on;
...
surfplot_rect(x_h_0, x_o_0, distance, size_veh, select1, 0);
contour(X_var,Y_var,apf_obst2,100);
hold off;
shading interp;
xlabel('Longitudinal Distance')
ylabel('Lateral Distance')
